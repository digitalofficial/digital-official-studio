...
export type GalleryLayout = 'masonry' | 'grid' | 'editorial'
...
export default function MasonryGrid({ items, showDownload = false, watermarkConfig, layout = 'masonry' }: Props) {
  ...
  // The photo/video box: natural height for masonry, a fixed frame for grid/editorial.
  const frameClass =
    layout === 'grid'
      ? 'relative aspect-square'
      : layout === 'editorial'
      ? 'relative aspect-[3/2] bg-navy'
      : 'relative'
  const imgClass =
    layout === 'masonry'
  ...
  function Tile({ item, idx }: { item: MediaItem; idx: number }) {
    ...
    return (
      <div className="mb-4 group relative">
        <div
          className={`rounded-lg overflow-hidden cursor-pointer ${frameClass}`}
          onClick={() => setLightboxIndex(idx)}
          onContextMenu={hasWatermark ? (e) => e.preventDefault() : undefined}
        >
          {isVideo ? (
            <div className={layout === 'masonry' ? 'relative aspect-video bg-card' : 'absolute inset-0 bg-card'}>
              <video
                src={item.file_url}
                muted
                playsInline
            ...
            <>
              {layout === 'masonry' ? (
                ...
                <Image
                  ...
                  fill
                  className={imgClass}
                  sizes={layout === 'editorial' ? '(max-width: 896px) 100vw, 896px' : '(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw'}
                />
              )}
        ...
        {item.name && <p className="text-text text-sm mt-2 font-medium">{item.name}</p>}
        {item.caption && <p className="text-muted text-xs mt-1">{item.caption}</p>}
      </div>
    )
  }
    ...
    <>
      {layout === 'masonry' ? (
        <Masonry breakpointCols={breakpoints} className="flex -ml-4 w-auto" columnClassName="pl-4 bg-clip-padding">
          {items.map((item, idx) => <Tile key={item.id} item={item} idx={idx} />)}
        </Masonry>
      ) : layout === 'grid' ? (
